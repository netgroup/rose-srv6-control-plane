...
# Create a new vertex collection named "hosts" if it does not exist.
# This returns an API wrapper for "hosts" vertex collection.
if topo_graph.has_vertex_collection('hosts'):
    hosts = topo_graph.vertex_collection('hosts')
else:
    hosts = topo_graph.create_vertex_collection('hosts')
...
# Create an edge definition named "host_links". This creates any missing
# collections and returns an API wrapper for "host_links" edge collection.
if topo_graph.has_edge_definition('host_links'):
    host_links = topo_graph.edge_collection("host_links")
else:
    host_links = topo_graph.create_edge_definition(
        edge_collection='host_links',
        from_vertex_collections=['routers', 'hosts'],
        to_vertex_collections=['hosts', 'routers']
    )
