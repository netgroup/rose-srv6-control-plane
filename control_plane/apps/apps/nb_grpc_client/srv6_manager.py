...
def handle_srv6_usid_policy(controller_channel, operation, nodes_dict,
    ...
    micro_sid.l_fwd_engine = nb_commons_pb2.FwdEngine.Value(
        l_fwd_engine.upper())
    ...
    micro_sid.r_fwd_engine = nb_commons_pb2.FwdEngine.Value(
        r_fwd_engine.upper())
...
def handle_srv6_behavior(controller_channel, operation, grpc_address,
    ...
    # Set the action
    srv6_behavior.action = nb_commons_pb2.SRv6Action.Value(
        utils.action_to_grpc_repr[action])
    # Set the device
    ...
    # Set the forwarding engine
    srv6_behavior.fwd_engine = nb_commons_pb2.FwdEngine.Value(
        fwd_engine.upper())
    #
    # Get the reference of the stub
    stub = nb_srv6_manager_pb2_grpc.SRv6ManagerStub(controller_channel)
...
def handle_srv6_unitunnel(controller_channel, operation, ingress_ip,
    ...
    # Set the forwarding engine
    srv6_unitunnel.fwd_engine = nb_commons_pb2.FwdEngine.Value(
        fwd_engine.upper())
    #
    # Get the reference of the stub
    stub = nb_srv6_manager_pb2_grpc.SRv6ManagerStub(controller_channel)
...
def handle_srv6_biditunnel(controller_channel, operation, node_l_ip,
    ...
    # Forwarding engine
    srv6_biditunnel.fwd_engine = nb_commons_pb2.FwdEngine.Value(
        fwd_engine.upper())
    #
    # Get the reference of the stub
    stub = nb_srv6_manager_pb2_grpc.SRv6ManagerStub(controller_channel)
